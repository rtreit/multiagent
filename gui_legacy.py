...
# Use 127.0.0.1 instead of localhost to avoid Windows DNS resolution delays
AGENT_BASE_URL = "http://127.0.0.1"

# Cache A2A clients to avoid recreating them on every request
_client_cache = {}
...
def chat():
    ...
    # GUI should make simple HTTP requests to agent web endpoints
    # NOT use A2A protocol - that's for agent-to-agent communication
    url = f"{AGENT_BASE_URL}:{AGENT_PORTS[agent]}"
    ...
    try:
        ...
        # Send a simple HTTP POST request with JSON payload
        # This is how web frontends should communicate with backend services
        task_data = {
            'message': text,
            'timestamp': time.time()
        }
        ...
        print(f"[GUI] Sending HTTP POST to {url}/message")
        ...
        print(f"[GUI] HTTP request took {send_request_time:.2f}s")
        ...
        if response.status_code == 200:
            ...
            return jsonify({
                ...
                'timing': {
                    ...
                    'http_request': f"{send_request_time:.2f}s"
                }
            })
        else:
            return jsonify({
                'success': False, 
                'error': f'Agent returned HTTP {response.status_code}: {response.text}'
            })
